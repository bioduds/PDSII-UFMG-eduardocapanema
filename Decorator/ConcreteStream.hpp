...
/**
 * \Este é o componente concreto do decorador. 
 Ele fornece uma implementação
 * muito simples para o fluxo abstrato.
 */
class ConcreteStream : public AbstractStream {
  public:
    /**
    * \Construtor. Ele recebe uma string e a liga à 
    string interna que essa classe
    * encapsula
    * \param stream: a string que será representada 
    por este objeto.
    */
    ConcreteStream( const std::string stream ) : _stream( stream ) {}
    /**
    * \simplesmente retorna a string interna. 
    Sem modificações.
    * \retornar a string não modificada encapsulada 
    por este objeto.
    */
